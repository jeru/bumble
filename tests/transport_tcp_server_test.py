...
class OpenTcpServerTransportTests(unittest.TestCase):
    ...
    def test_open_with_normal_spec(self):
        asyncio.run(open_tcp_server_transport('localhost:32100'))
        self.mock_create_server.assert_awaited_once_with(
            ANY, host='localhost', port=32100, sock=None
        )

    def test_open_with_port_only_spec(self):
        asyncio.run(open_tcp_server_transport('_:32100'))
        self.mock_create_server.assert_awaited_once_with(
            ANY, host=None, port=32100, sock=None
        )

    def test_open_without_spec_nor_sock(self):
        with self.assertRaises(ValueError):
            asyncio.run(open_tcp_server_transport(spec=None))
        self.mock_create_server.assert_not_awaited()

    def test_open_with_spec_and_sock(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            with self.assertRaises(ValueError):
                asyncio.run(open_tcp_server_transport(spec='_:32100', sock=sock))
            self.mock_create_server.assert_not_awaited()

    def test_open_with_sock(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            asyncio.run(open_tcp_server_transport(sock=sock))
        self.mock_create_server.assert_awaited_once_with(
            ANY, host=None, port=None, sock=sock
        )
